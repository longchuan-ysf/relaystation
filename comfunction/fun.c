 ...
 **/
 void printf_Hexbuffer(uint8_t *buffer,uint32_t len)
 {
	...
	uint8_t   temp[8];
	uint8_t *printbuf;
	uint32_t buf_len = len*5+len/16+8;
	//每个hex会输出成0x×× 占用5个字符空间，然后16个增加一个换行最后前后插入换行，应该是+2.但是+8留一定的余量
	printbuf = (uint8_t *)malloc(buf_len);
	memset(temp,0,sizeof(temp));
	memset(printbuf,0,buf_len);
	strcat((char *)printbuf,"\n");
	for(i=0;i<len;i++)
	 {
		memset(temp,0,8);
		sprintf((char  *)&temp,"0x%02x ",buffer[i]);
		//printf("%s",temp);
		strcat((char *)printbuf,(char *)temp);
		if(((i+1)%16 == 0))
		{
 ...
 unsigned  char CheckSum(const unsigned  char *buffer,unsigned  int bufferLen)
{
 ...
 /**
****************************************************************************************
@brief:    DigitalToString  数字转换成字符串
@Input：   Digital 数据		   
@Output：  String  接收最终输出的字符串
@Return    MYERR_GENERAL有错误 MYERR_NONE OK
@Warning:  NULL
@note:     龙川2019-4-15
****************************************************************************************
 **/
uint8_t DigitalToString(uint32_t Digital,uint8_t *String)
{
	uint8_t Temp,len,i;
	if(String == NULL)
		return -EINVAL;
...
*			  len:指定字符串数组的长度——超过这个长度后自动返回防止数组越界
*	返 回 值: 二进制整数值
*********************************************************************************************************
*/
int str_to_int(char *_pStr,unsigned char len)
{
	unsigned char flag;
	char *p;
	int ulInt;
...
uint32_t AsciiToUint32(char *pAscii)
{
	unsigned char i;
	unsigned char bTemp;
	unsigned char bIsHex;
	unsigned char bLen;
	unsigned char bZeroLen;
	uint32_t lResult;
	uint32_t lBitValue;
